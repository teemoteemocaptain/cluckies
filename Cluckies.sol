...
// File: contracts/Cluckies.sol


pragma solidity ^0.8.0;
contract TheCluckies  is ERC721Enumerable, Ownable {
	using Strings for uint256;
    ...
    mapping(uint256 => mapping(address => uint256)) private _holdStage;

	constructor(
        string memory _baseMetadataUrl
	)
	ERC721("TheCluckies", "CLUCK") {
    ...
    function getStage(uint256 tokenId, address wallet) public view returns (uint256) {
        if(_holdStage[tokenId][wallet] == 0) {
            return 0;
        } else {
            uint256 holdDays = (block.timestamp - _holdStage[tokenId][wallet]) / 1 days;
            if(holdDays > 4) {
                return 4;
            } else {
                return holdDays;
    ...
    function transferFrom(address from, address to, uint256 tokenId) public override(IERC721, ERC721) {
        ...
        _holdStage[tokenId][to]= block.timestamp - (getStage(tokenId, from) * 1 days);
...
//  We will never get greedy and increase supply, this function is made to reduce max supply if we dont oos or for other reasons
	function reduceMaxSupply(uint256 _newMaxSupply) external onlyOwner {
        require(_newMaxSupply <= maxSupply, "Can't Increase max supply");
